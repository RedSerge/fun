...
def update_model(model, target_model, replay_memory, batch_size=BATCH_SIZE):
    ...
    # Double DQN: Argmax applied to the original model, but real values extracted from the target one. 
    max_indices=np.argmax(model.predict(s2), axis=1)
    s2_values=[]
    for i,m in enumerate(target_model.predict(s2)):
        s2_values+=[m[max_indices[i]]]
